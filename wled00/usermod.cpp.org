...
//        Serial.println("Checking for UDP Microphone Packet");
        ...
        if (packetSize) {
          ...
          if (!(isValidUdpSyncVersion(packetHeader))) {
            ...
            // Only change reactivePeak IF it's currently false.  If it's true already, then the animation still needs to respond
            if (!reactivePeak) {
              reactivePeak = receivedPacket.reactivePeak;
            }
