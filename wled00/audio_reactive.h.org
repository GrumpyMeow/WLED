...
bool reactivePeak = false;                            // Boolean flag to indicate a peak/beat has been detected . Effects should reset this flag if processed.
int sampleAdj;                                      // Gain adjusted sample value.
#ifdef ESP32                                        // Transmitting doesn't work on ESP8266, don't bother allocating memory
   bool udpSamplePeak = false;                      // Boolean flag for peak. Set at the same time as reactivePeak, but reset by transmitAudioData
#endif
...
////////////////////
  ...
  void transmitADTimer_callback(void *arg)
  {
    if (audioSyncEnabled & (1 << 0)) {
      ...
      transmitData.reactivePeak = udpReactivePeak;
      udpReactivePeak = 0;                              // Reset udpReactivePeak after we've transmitted it
...
void logAudio() {
  ...
  //------------ Oscilloscope output ---------------------------
    ...
    Serial.print(reactivePeak); Serial.print(" ");    //reactivePeak = 0;
