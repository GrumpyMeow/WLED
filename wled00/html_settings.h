...
body{text-align:center;background:#222;height:100;margin:0}html{--h:10.00vh}button{background:#333;color:#fff;font-family:Verdana,Helvetica,sans-serif;border:.3ch solid #333;display:inline-block;font-size:8vmin;height:var(--h);width:95%;margin-top:2.4vh}
</style><script>
...
function H(){window.open("https://github.com/Aircoookie/WLED/wiki/Settings#sound-settings")}function B(){window.open("/settings","_self")}function GetV() {var d=document;
%CSS%%SCSS%</head><body onload="GetV()">
<form id="form_s" name="Sf" method="post"><div class="helpB"><button 
type="button" onclick="H()">?</button></div><button type="button" onclick="B()">
Back</button><button type="submit">Save</button><hr><h2>Sound setup</h2><h3>
Sound settings</h3>Enable: <input type="checkbox" name="SE"><br>Sample-rate: 
<select name="SR"><option value="44100" selected="selected">44100 Hz</option>
<option value="22050">22050 Hz</option><option value="11025">11025 Hz</option>
<option value="8000">8000 Hz</option><option value="5512">5512 Hz</option>
<option value="1000">1000 Hz</option></select><br><button type="button" 
onclick="B()">Back</button><button type="submit">Save</button></form></body>
</html>)=====";
